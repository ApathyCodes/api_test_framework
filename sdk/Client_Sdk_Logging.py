...
class ClientSDK:  # 建类
    ...
    def do_requests(self, method: str, url: str, desc=None, params=None, data=None, **kwargs):
        ...
        # 手动print日志
        # print(f'{method}请求  请求路径：{url}---{desc}')  加入logging调试
        logging.info(f'[{method}请求]  请求路径：{url}---{desc}')

        if method == "get":
           res = requests.get(url, params=params, **kwargs)
           # 手动print状态码

           # print(f'get请求数据:{params}---状态码：{res.status_code} ')  加入logging调试
           logging.info(f'get请求数据:{params}---状态码：{res.status_code} ')


        elif method == 'post':
            res = requests.post(url, data=data, **kwargs)
            # 手动print状态码
            # print(f'post请求数据:{data}---状态码：{res.status_code} ')  加入logging调试
            logging.info(f'post请求数据:{data}---状态码：{res.status_code} ')
